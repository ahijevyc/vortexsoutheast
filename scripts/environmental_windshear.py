...
def main():
    ...
    parser.add_argument(
        ...
        "--debug",
        action="store_true",
        help="more debug messages and pdb.set_trace before ending",
    )
    parser.add_argument(
        "-n",
        action="store_true",
        help="list missing storm and times but do not fill them",
    )
    parser.add_argument(
        "ifile",
    ...
    out = df.groupby(["stormname", "narrtime"], group_keys=False).apply(
        getTCFlow, all_tracks, args
    )
    wind_shear_vector = metpy.calc.wind_components(
        out["wind_shear_speed"].values * units.m / units.s,
        (out["wind_shear_heading"].values + 180.0) * units.deg,
    )
    storm_motion_vector = metpy.calc.wind_components(
        out["storm_motion_speed"].values * units.m / units.s,
        (out["storm_motion_heading"].values + 180.0) * units.deg,
    )
    # shear_x_motion is positive when wind shear points to right of storm motion.
    shear_x_motion = np.cross(
        np.array(wind_shear_vector).T, np.array(storm_motion_vector).T
    )
    out["shear_x_motion"] = shear_x_motion
    out["shear_rt_of_motion"] = out["shear_x_motion"] > 0
    out["shear_x_motion_norm"] = (
        shear_x_motion / out.wind_shear_speed / out.storm_motion_speed
    )
    ...
    u, v = metpy.calc.wind_components(
        avg.wind_shear_speed * units.meter / units.second,
        avg.wind_shear_heading * units.deg + 180 * units.deg,
    )
    out["year"] = pd.to_datetime(out.init, format="%Y-%m-%d_%H:%M:%S").dt.year
    print(
        f"{os.path.basename(ifile).replace('.0003060912151821z.txt','').center(53)}  "
...
def getTCFlow(df, all_tracks, args):
    ...
    file_ncl = narr.get(
        narrtime, narrtype=narr.narr3D, targetdir="/glade/scratch/ahijevyc/NARR"
    )
    imatch = (all_tracks["stormname"] == stormname.upper()) & (
    ...
    if os.path.exists(TCFlow_csv):
        ...
        logging.warning(
            f"no {stormname} {narrtime.strftime(fmt)} TCFlow csv file {TCFlow_csv} making it."
        )
        if args.n:
            return

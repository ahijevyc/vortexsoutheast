...
def getparser():
    parser = argparse.ArgumentParser(
        description="skew t of output from NARR",
    ...
    parser.add_argument(
        ...
        help="input csv file with storm,initialization time,shr,station",
    )
    parser.add_argument(
        "--no-fineprint",
        action="store_true",
    ...
    parser.add_argument(
        "--pmin", type=int, default=100, help="mask values below this pressure"
    )
    return parser
...
def main():
    ...
    df = pd.read_csv(
        ifile, names=["storm", "itime", "place", "station"], parse_dates=["itime"]
    )
    assert (
        df["place"].isin(["upshr", "dnshr"]).all()
    ), "expected place to be upshr or dnshr"
    ...
    for index, row in df.iterrows():
        ...
        if os.path.exists(cache):
            ...
            obs = obs.drop_duplicates(
                subset="pressure", ignore_index=True
            )  # Avoid ValueError: Values in `t_eval` are not properly sorted.
            udict = {
                "pressure": "hPa",
                "height": "meter",
                "temperature": "degC",
        ...
        distance_from_TCcenter, bearing_from_TCcenter = dist_bearing(
            TC.lon * units.deg, TC.lat * units.deg, np.array(slon), np.array(slat)
        )
        ...
        for stype in ["NARR", "obs"]:
            ...
            if stype == "obs":
                ...
                relative_humidity = mpcalc.relative_humidity_from_dewpoint(T, Td).to(
                    "percent"
                )
                (od["0degC rh"],) = interpolate_1d(
                    0 * units.parse_expression("degC"), T, relative_humidity
                )
                lon, lat = slon, slat
                zs, T0s, Td0s = interpolate_1d(
                    sigps, p, height, T, Td
                )  # ignore UserWarning: Interp pt out of data bounds... they're set to nan.
                for z, T0, Td0, sigp in zip(zs, T0s, Td0s, sigps):
                    ...
                    od[f"{sigp:.0f} thetae"] = mpcalc.equivalent_potential_temperature(
                        sigp, T0, Td0
                    ).to("degC")
            else:
                ...
                x, y = narr.data_crs.transform_point(
                    slon.m, slat.m, src_crs=cartopy.crs.PlateCarree()
                )
                ...
                if not prof.equals(ds.metpy.sel(**nearest)):
                    logging.warning("sel method pulled different profile than imin")
                    pdb.set_trace()

                lon = prof.gridlon_221.metpy.quantify().data
                lat = prof.gridlat_221.metpy.quantify().data
                p = prof.lv_ISBL0
                T = prof["TMP_221_ISBL"].metpy.convert_units(
                    "degC"
                )  # skew.plot() assumes T is in degC
                height = prof["HGT_221_ISBL"]
                Td = mpcalc.dewpoint_from_specific_humidity(
                    p, T, prof["SPF_H_221_ISBL"]
                )
                ...
                for sigp in sigps:
                    ...
                    skew.ax.text(
                        0.01,
                        sigp,
                        f"{z.astype(int).metpy.quantify().data:~}",
                        fontsize="small",
                        transform=trans,
                        ha="left",
                        va="center",
                ...
                od["narr sbcape"] = (
                    narr.scalardata("sbcape", itime).isel(imin).data.compute()
                )
                od["narr sbcinh"] = (
                    narr.scalardata("sbcinh", itime).isel(imin).data.compute()
                )
                od["narr mlcape"] = (
                    narr.scalardata("mlcape", itime).isel(imin).data.compute()
                )
                od["narr mlcinh"] = (
                    narr.scalardata("mlcinh", itime).isel(imin).data.compute()
                )
                od["narr 0degC rh"] = (
                    narr.scalardata("rh_0deg", itime).isel(imin).data.compute()
                )
                ...
                od["pwat"] = (
                    narr.scalardata("pwat", itime).isel(imin).data.compute()
                    * units.m**3
                    / (1000 * units.kg)
                )
                od["pwat"] = od["pwat"].to("mm")
                logging.debug("NARR shears")
                od["narr shr10m_900hPa"] = (
                    narr.scalardata("shr10m_900hPa", itime).isel(imin).data.compute()
                )
                od["narr shr10m_700hPa"] = (
                    narr.scalardata("shr10m_700hPa", itime).isel(imin).data.compute()
                )
                od["narr shr10m_1000m"] = (
                    narr.scalardata("shr10m_1000m", itime).isel(imin).data.compute()
                )
                od["narr shr10m_3000m"] = (
                    narr.scalardata("shr10m_3000m", itime).isel(imin).data.compute()
                )
            ...
            p, T, Td, profT = mpcalc.parcel_profile_with_lcl(
                p, T, Td
            )  # not virtual temperature yet
            if (p[0], profT[0]) == (p[1], profT[1]):
                assert (T[0], Td[0]) == (
                    T[1],
                    Td[1],
                ), "bottom pressure repeated but (T,Td) differ"
            ...
            mixing_ratio_parcel = mpcalc.mixing_ratio_from_relative_humidity(
                p[parcel_level], T[parcel_level], relative_humidity[parcel_level]
            )
            prof_mixing_ratio = mpcalc.mixing_ratio_from_relative_humidity(
                p, profT, 1
            )  # saturated mixing ratio
            od["lcl_pressure"], _ = mpcalc.lcl(
                p[parcel_level], T[parcel_level], Td[parcel_level]
            )
            prof_mixing_ratio[p >= od["lcl_pressure"]] = (
                mixing_ratio_parcel  # unsaturated mixing ratio (constant up to LCL)
            )
            ...
            mixing_ratio = mpcalc.mixing_ratio_from_relative_humidity(
                p, T, relative_humidity
            )
            cold_and_probably_dry = np.isnan(relative_humidity) & (T < -40 * units.degC)
            if np.any(cold_and_probably_dry):
                logging.warning(
                    f"replace {cold_and_probably_dry.sum()} missing mixing ratios with zero where T < -40C"
                )
            ...
            storm_u = 0.0 * units.m / units.s
            storm_v = 0.0 * units.m / units.s
            ...
            srh01_pos, srh01_neg, srh01_tot = mpcalc.storm_relative_helicity(
                height, u, v, 1 * units.km, storm_u=storm_u, storm_v=storm_v
            )
            ...
            od["lowest level thetae"] = mpcalc.equivalent_potential_temperature(
                p[0], T[0], Td[0]
            )
            ...
            od["storm_u"] = storm_u
            od["storm_v"] = storm_v
            od["metpy sfc-5km shr mag"] = mpcalc.wind_speed(
                *mpcalc.bulk_shear(
                    p, u, v, height, bottom=height.min(), depth=5 * units.km
                )
            )
            od["metpy sfc-3km shr mag"] = mpcalc.wind_speed(
                *mpcalc.bulk_shear(
                    p, u, v, height, bottom=height.min(), depth=3 * units.km
                )
            )
            od["metpy sfc-1km shr mag"] = mpcalc.wind_speed(
                *mpcalc.bulk_shear(
                    p, u, v, height, bottom=height.min(), depth=1 * units.km
                )
            )
            ...
            for q, path in zip(
                [0.4, 1, 2, 4, 7, 10, 16, 24, "32g/kg"], lines.get_paths()
            ):
                skew.ax.text(
                    *path.vertices[-3],
                    q,
            ...
            for label_hgt in label_hgts:
                ax_hod.text(
                    np.interp(label_hgt, height, u),
                    np.interp(label_hgt, height, v),
                    label_hgt.to("km").m,
                    fontsize=7,
                )
                ax_hod.plot(
            ...
            ofile = os.path.join(
                odir, f"{itime.strftime('%Y%m%dT%H%M')}.{station}.{stype}.skewt.png"
            )
            plt.savefig(ofile)
            logging.info(f"made {os.path.realpath(ofile)}")
            fig.clear()
            odf = pd.concat(
                [odf, pd.DataFrame([od])], ignore_index=True
            )  # Avoid FutureWarning about append. Keep od as dictionary (don't convert to DataFrame)

    logging.info("move units from values to column names")

...
def main():
    parser = argparse.ArgumentParser(
        description="skew t of output from NARR_composite.py",
    ...
    parser.add_argument(
        ...
        default="torn max",
        help="predetermined list of points by coord and centroid",
    )
    parser.add_argument(
        "--coord",
        type=str,
    ...
    parser.add_argument(
        "--idir",
        type=str,
        default="/glade/scratch/ahijevyc/trier/VSE/nc",
        help="input directory",
    )
    parser.add_argument(
        "--odir", type=str, default="../output", help="output directory"
    )
    ...
    for f in files:
        assert os.path.exists(
            f
        ), f"{os.path.realpath(f)} does not exist. Did u remember to run ~/bin/stack_NARR.csh?"
    ...
    ds = ds.metpy.quantify().mean(
        dim="storm"
    )  # if you don't quantify() you lose units.
    ...
    for point in points:
        ...
        narr = ds.sel(range=range_km, azimuth=azimuth, method="nearest").sel(
            coord=coord
        )  # coord is a string. "nearest" method undefined.
        cape = capeds.sel(range=range_km, azimuth=azimuth, method="nearest").sel(
            coord=coord
        )
        srh = (
            xarray.open_dataset(f"{idir}/{desc}.srh.{hrs}.nc")
            .sel(range=range_km, azimuth=azimuth, method="nearest")
            .sel(coord=coord)
        )
        ...
        p, T, Td, prof = mpcalc.parcel_profile_with_lcl(
            p, T, Td

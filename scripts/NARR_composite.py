...
def rotate_vector(vector, az):
    ...
    u = vector.isel(
        uv=0
    ).copy()  # copy() is VERY important or else u remains a reference to uv and is changed by first .loc command below
...
def add_rgrid(ax, ring_interval=200):
    ax.grid(
        alpha=0.4, linewidth=0.4, color="k"
    )  # tried moving outside loop right after figure was created but grid was not visible (even with zorder=3)
    ...
    for i in range(len(rlabels) - 1):
        rlabels[i] = ""
    logging.debug(
        f"add_rgrid: ring_interval={ring_interval} start={start}, stop={stop} radii={radii} {rlabels}"
    )
    lines, labels = ax.set_rgrids(
        radii, labels=rlabels, angle=0, ha="center", va="bottom", fontsize=4.5
    )  # va="baseline" puts label too close to ring
    return lines, labels
...
def get_normalize_range_by(track, normalize_by):
    ...
    if normalize_by == "r34":
        ...
        data = narr.vectordata("wind10m", track["valid_time"])
        lon, lat = data.metpy.longitude, data.metpy.latitude
        u, v = data
        derived_vitals_dict = atcf.derived_winds(
            u, v, xarray.full_like(u, 1013.0) * units("hPa"), lon, lat, track
        )
        storm_size_S = derived_vitals_dict["storm_size_S"]
        if np.isnan(storm_size_S):
            logging.info(
                "storm_size_S is nan. This may be for Isaac 2012, which has artificial lat/lon extension, but no vmax"
            )
        ...
        if storm_size_S < 0.25:
            ...
            storm_size_S = 0.25
        logging.info(
            f'normalizing range by Knaff_Zehr S. {track["lat"]:.2f}N Vmax {track["vmax"]:.2f}'
        )
        logging.debug(f'Vmax from NARR (not used) {derived_vitals_dict["raw_vmax"]}')
        logging.info(
            f'Vt_500km={derived_vitals_dict["Vt_500km"]:.2f}  S={storm_size_S:.2f}'
        )
        ...
        if np.isnan(value):
            if normalize_by == "rmw":
                value = 25.0  # 25 nautical miles is default rmw in aswip.
            else:
                logging.error(
                    f"get_normalize_range_by(): Null value for {normalize_by}. Not sure how to define"
                )
                sys.exit(1)
...
parser.add_argument(
    "--cart", action="store_true", help="plot cartopy version (not TC-centric)"
)
parser.add_argument(
    "--clevels",
    type=float,
    nargs="+",
    default=[500, 1000, 2000, 3000, 4000, 5000, 10000],
    help="line contour levels",
)
parser.add_argument(
...
parser.add_argument(
    "-f", "--fill", default="shr10m_700hPa", help="variable name for contour fill field"
)
parser.add_argument("-l", "--line", help="variable name for line contour field")
parser.add_argument(
    "--max_range", type=float, default=800.0, help="maximum range in km"
)
parser.add_argument(
    "--narrdir",
    default=os.path.join("/glade/scratch", os.getenv("USER"), "NARR"),
    help="directory to untar NARR into",
)
parser.add_argument("--netcdf", help="netCDF file to write composite to")
parser.add_argument(
    "--no-fineprint", action="store_true", help="Don't write details at bottom of image"
)
...
parser.add_argument(
    "--show_storm_rpt_scatter",
    action="store_true",
    help="show each storm report as a symbol, not just density",
)
parser.add_argument(
    "--spctd",
    type=float,
    default=1.5,
    help="hours on either side of valid time to show SPC storm reports",
)
...
if not clobber and os.path.isfile(ofile) and netcdf and os.path.isfile(netcdf):
    logging.info(
        f"{ofile} and {netcdf} exist. Skipping. Use --clobber option to override."
    )
    sys.exit(0)
...
linecontour_alpha = 0.6
linecontour_fontsize = 4
barblength = 3.5
barbunits = "m/s"
barb_increments = {"half": 2.5, "full": 5, "flag": 25}  # for matplotlib.quiver.barb
barbkwdict = dict(
    length=barblength, linewidth=0.2, alpha=0.6, barb_increments=barb_increments
)
...
r_center2D, theta_center2D = np.meshgrid(
    range_center1D, theta_center1D
)  # one less element in range and azimuth than range_center1D
...
for istorm, storm in enumerate(stormlist):
    ...
    trackdf, best_track_file = ibtracs.get_df(
        stormname, year, basin="NA"
    ...
    logging.info(
        f"{stormname_year} {trackdf.valid_time.min()} to {trackdf.valid_time.max()} ({duration})"
    )

    if narrtime > trackdf.valid_time.max():
        logging.warning(
            f"requested NARR time {narrtime} is after {stormname_year} ends"
        )
    ...
    assert (
        trackdf.valid_time.min() <= narrtime <= trackdf.valid_time.max()
    ), f"requested time {narrtime} is outside TC track time window."
    ...
    snapshot = os.path.join(
        ...
        + f'.{valid_time.strftime("%Y%m%d%H")}.{lat1.m:04.1f}N{lon1.m:06.1f}E{ext}',
    )
    supt = figplr.suptitle(
    ...
    storm_report_time_window_str = (
        "storm reports\n"
        + storm_report_start.strftime("%m/%d %H%M")
        + " - "
        + storm_report_end.strftime("%m/%d %H%M %Z")
    )
    storm_report_window = (all_storm_reports["time"] >= storm_report_start) & (
        all_storm_reports["time"] < storm_report_end
    )
    storm_reports = all_storm_reports.loc[
        storm_report_window
    ].copy()  # avoid SettingWithCopyWarning in spc.polarkde by applying .copy()
    ...
    assert (
        dist_from_center.min() <= 32 * units.km
    ), f"at {valid_time} {stormname_year} > {dist_from_center.min():~.1f} from nearest NARR grid point"
    ...
    logging.debug(
        "creating weighted and unweighted 2d histograms of theta and range..."
    )

    values = spc.histogram2d_weighted(
        bearing, dist_from_center, azbins, rbins, data
    )  # Don't apply uvsel() here; we need values for storm motion and wind shear

    logging.info(f"steering flow at {lat1} {lon1}")
    file_ncl = narr.get(valid_time, narrtype=narr.narr3D, targetdir=narrdir)
    rx = 4.5
    TCFlow_csv = os.path.join(
        os.path.dirname(file_ncl),
    ...
    wind_shear_heading = (
        TCFlow["wind_shear_heading"].values[0] * units.deg
    )  # units needed below
    ...
    ax_labels = {
        northax: ("N", "", "E", "", "S", "", "W", ""),
        stormmotionax: (
            f"storm heading {storm_heading.m:.0f}$\degree$ at {storm_speed:~.1f}\nfront",
            "",
            "R",
            "",
            "rear",
        ...
        windshearax: (
            f"wind shear heading {wind_shear_heading.m:.0f}$\degree$ at {wind_shear_mag:~.1f}\ndownshear",
            "",
            "R",
            "",
            "upshear",
    ...
    for ax, axheading in ax_headings.items():
        ax.grid(
            False
        )  # Auto-removal of grids by pcolor() and pcolormesh() is deprecated since 3.5 and will be removed two minor releases later; please call grid(False)
        ...
        )  # tried DataArray.plot.imshow and pcolormesh, tried 1D coordinates.
        filldict[ax].append(rotated_values)
        if line:
            linecontour_values = spc.histogram2d_weighted(
                bearing, dist_from_center, azbins, rbins, linecontourdata
            )  # Don't apply uvsel() here; we need values for storm motion and wind shear
            linecontour_values = roll_rotate_sel(linecontour_values, axheading)
            polarc = ax.contour(
                np.radians(theta_center1D),
                range_center1D,
                linecontour_values.T,
                lcontour_levels,
                colors="black",
                alpha=linecontour_alpha,
            )
            ax.clabel(polarc, fontsize=linecontour_fontsize, fmt="%.0f")
            linedict[ax].append(linecontour_values)
        if barb:
            barb_values = spc.histogram2d_weighted(
                bearing, dist_from_center, azbins, rbins, barbdata
            )
            barb_values = roll_rotate_sel(barb_values, axheading)
            polarb = ax.barbs(
                np.radians(theta_center2D.ravel()[ii]),
                r_center2D.ravel()[ii],
                *barb_values.stack(gate=["azimuth", "range"]).isel(gate=ii),
                **barbkwdict,
            )
            barbdict[ax].append(barb_values)
        if quiver:
            quiver_values = spc.histogram2d_weighted(
                bearing, dist_from_center, azbins, rbins, quiverdata
            )
            quiver_values = roll_rotate_sel(quiver_values, axheading)
            polarq = ax.quiver(
                np.radians(theta_center2D.ravel()[ii]),
                r_center2D.ravel()[ii],
                *quiver_values.stack(gate=["azimuth", "range"]).isel(gate=ii),
            )
            quiverdict[ax].append(quiver_values)
            powerof10 = 10 ** int(
                np.log10(np.sqrt(quiverdata[0] ** 2 + quiverdata[1] ** 2).max().values)
            )  # order of magnitude
            ax.quiverkey(
                polarq,
                0.1,
                -0.05,
                powerof10,
                f"{powerof10} {quiver_values.metpy.units:~}",
                labelpos="S",
        ...
        if not storm_reports.empty:
            ...
            storm_reports = spc.polarplot(
                lon1,
                lat1,
                storm_reports,
                ax,
                zero_azimuth=axheading,
                add_legend=ax == northax,
                legend_title=storm_report_time_window_str,
            )
            storm_rpt_dict[ax] = pd.concat([storm_rpt_dict[ax], storm_reports])
            if ax is northax:
                logging.info(
                    f"found {len(storm_reports)} storm reports within {max_range}. {storm_reports['significant'].sum()} significant."
                )
            ...
            if showkde and len(storm_reports) >= 3:
                ...
                for event_type in rptkde:
                    ...
                    ax.set_title(
                        f"{desc(data)} {event_type} r={rho:.3f}", fontsize="xx-small"
                    )

        fontsize = "large" if ax == northax else "x-small"
        _, _ = ax.set_thetagrids(
            theta_lines, labels=ax_labels[ax], fontweight="demi", fontsize=fontsize
        )
    ...
    os.system(
        "mogrify -colors 128 " + snapshot
    )  # severe drop in quality with 64 colors
...
figplr.suptitle(
    ", ".join(sorted_storms) + "  " + hours_str, fontsize=fontsize
)  # Clean title w/o origin place and time
...
assert (
    polarf.get_array().size == np.mean(filldict[northax], axis=0).size
), "filled contour field changed size. Check for nans"
for ax in [northax, stormmotionax, windshearax]:
    ...
    if barb:
        ax.barbs(
            np.radians(theta_center2D.ravel()[ii]),
            r_center2D.ravel()[ii],
            *xr_list_mean(barbdict[ax]).stack(gate=["azimuth", "range"]).isel(gate=ii),
            **barbkwdict,
        )
    if quiver:
        ...
        polarq = ax.quiver(
            np.radians(theta_center2D.ravel()[ii]),
            r_center2D.ravel()[ii],
            *xr_list_mean(quiverdict[ax])
            .stack(gate=["azimuth", "range"])
            .isel(gate=ii),
        )
    ...
    for event_type, xrpts in storm_rpt_dict[ax].groupby("event_type"):
        ...
        labels.append(
            f"{event_type} ({nrpts})"
        )  # append total number of events to legend label
        logging.info(
            f"centroid of {ax} coord {nrpts} {event_type} reports\n{spc.centroid_polar(lsr_heading, lsr_range)}"
        )
        if n_unique_locs < 3:
            if ax is northax:
                logging.info(
                    f"{n_unique_locs} unique {event_type} report locations. Not enough for kde (requires >= 3 non-colinear locations)"
                )
        ...
        ds = 2
        X, Y = np.mgrid[
            -max_range : max_range + ds : ds, -max_range : max_range + ds : ds
        ]
        az = pd.DataFrame(
            np.degrees(np.arctan2(Y, X))
        )  # Make DataFrame because histogram2d_weighted expects a .values attribute
        ...
        if event_type == "torn":
            weights = (
                xrpts["mag"].fillna(value=0) + 1
            )  # F-scale plus one. (F-sum from McCaul, 1991) # unknown tornado F-scales exist.
            event_type = "torn F-sum"
        ...
        kde = (
            kde * weights.sum()
        )  # Multiply kde (a normalized surface with total volume = 1) by the sum of weights (not the count)
        kde = xarray.DataArray(
            kde / lsr_range.units**2 / time_window_sum
        ...
        kde["azimuth"] = np.radians(
            kde.azimuth
        )  # Polar Axes are in radians not degrees.
        kdelevels = [0.00002, 0.00004, 0.00008, 0.00016, 0.00032, 0.00064]
        if kde.data.max() > kdelevels[0] * units.parse_expression(
            "1/km**2/day"
        ):  # avoid ZeroDivisionError with add_colorbar
            logging.info(f"max density {kde.data.max()} plotting kde")
            try:
                polarc = kde.plot.contour(
                    x="azimuth",
                    y="range",
                    ax=ax,
                    levels=kdelevels,
                    colors=["black"],  # colors is 1-element list
                    linewidths=0.5,
                    add_colorbar=True,
                    cbar_kwargs={"shrink": 0.75, "pad": 0.09},
                )
                cb = polarc.colorbar
                ...
                cb.set_label(
                    event_type + " " + cb.ax.yaxis.get_label().get_text(),
                    fontsize="xx-small",
                )
                cb.ax.tick_params(labelsize="xx-small")
            except Exception:
                logging.info("error with kde.plot.contour")

        else:
            logging.info(
                f"max density {kde.data.max()} <= first kde level {kdelevels[0]}. skipping kdeplot"
            )
        ...
        w[r_center2D * units.km >= rho_limit] = 0.0
        rho_limited = corr(fillmean, kde, w)
        logging.info(
            f"{fill} {nrpts} {event_type} r={rho:.3f} r{rho_limit:~}={rho_limited:.3f}"
        )
        ax.set_title(
            f"{desc(data)} {nrpts} {event_type} r={rho:.3f} r{rho_limit:~}={rho_limited:.3f}",
            fontsize=4,
            wrap=True,
        )
...
_, _ = northax.set_thetagrids(
    theta_lines,
    labels=("N", "", "E", "", "S", "", "W", ""),
    fontweight="demi",
    fontsize="large",
)
_, _ = stormmotionax.set_thetagrids(
    theta_lines,
    labels=("front", "", "R", "", "rear", "", "L", ""),
    fontweight="demi",
    fontsize=11,
)
_, _ = windshearax.set_thetagrids(
    theta_lines,
    labels=("downshear", "", "R", "", "upshear", "", "L", ""),
    fontweight="demi",
    fontsize=11,
)
...
if netcdf:
    ...
    if line:
        ds_dict[line] = (("coord", "storm", "azimuth", "range"), coord_stack(linedict))
    if barb:
        ds_dict[barb] = (
            ("coord", "storm", "component", "azimuth", "range"),
            coord_stack(barbdict),
        )
    if quiver:
        ds_dict[quiver] = (
            ("coord", "storm", "component", "azimuth", "range"),
            coord_stack(quiverdict),
        )
    ds = xarray.Dataset(
        ds_dict,
        coords={
            "coord": ["north", "storm motion", "wind shear"],
            "range": range_center1D,
            "azimuth": theta_center1D,
            "storm": stormname_years,
            "component": ["u", "v"],
        },
    )
    ...
    if "vertical" in attrs:
        try:
            ...
            attrs["vertical"] = str(
                attrs["vertical"]
            )  # convert pint Quantity to str or get TypeError: Invalid value for attr ...
    ds[fill].attrs = attrs
    ds["range"].attrs = {"units": "km", "long_name": "range from origin"}
    ds["azimuth"].attrs = {
        "units": "degrees",
        "long_name": "degrees clockwise from north",
    }
    ds.attrs["daz"] = daz
    ds.attrs["dr"] = dr

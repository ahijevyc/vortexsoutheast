...
parser.add_argument("ifiles", nargs='+', help="input text files. each line starts with a storm name, followed by a yyyymmddhh time")
...
stormrpts["day"] = ( stormrpts.time.dt.time < datetime.time(hour=1, minute=30) ) | ( stormrpts.time.dt.time >= datetime.time(hour=13, minute=30) )
...
hh = countblock(hh)
cat_type = CategoricalDtype(categories=["0","1-4","5-9","10-19","20-49","50+"], ordered=True)

hh = hh.astype(cat_type)
sns.displot(data=hh.reset_index(), x="tornadoes", kind="hist", multiple="stack", hue="time", 
        hue_order=["night","day","n/a"], col="category", col_wrap=3)
